"""Snapshot-golden infrastructure (ported from the vszip suite).
...
DEVICE NOTE (vszipcl is OpenCL): a golden captures the output of ONE OpenCL
device. On the same device the plugin is deterministic (num_streams 1-vs-4 is
bit-identical), and the kernels are JIT-compiled from source so the Zig
optimize level (Debug vs ReleaseFast) does not change the math — so goldens are
stable per machine. They are NOT portable across devices/drivers (a GPU's SFU
trig / rounding differs from pocl's CPU device). Goldens are therefore committed
as dev-GPU regression anchors and are marked `@pytest.mark.golden`; CI on a
different OpenCL device runs `-m "not golden"` (the device-independent parity +
contract + validation tests). Regenerate goldens on the reference GPU.
