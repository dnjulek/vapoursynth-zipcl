...
static void rdft_16(float *data) {
...
static void dft_1(float *data, int stride) {
}

static void dft_3(float *data, int stride) {
...
static void dft_5(float *data, int stride) {
...
static void dft_7(float *data, int stride) {
...
static void dft_16(float *data, int stride) {
...
static void idft_1(float *data, int stride) {
}

static void idft_3(float *data, int stride) {
...
static void idft_5(float *data, int stride) {
...
static void idft_7(float *data, int stride) {
...
static void idft_16(float *data, int stride) {
...
static void irdft_16(float *data) {
...
static float to_float(TYPE x) {
    return convert_float(x) * (float)(SCALE);
}

static TYPE from_float(float x) {
#ifdef PEAK
    x = native_divide(x, (float)(SCALE));
    x = fmax(0.0f, fmin(x + 0.5f, (float)(PEAK)));
    return (TYPE)((int)(x));
#else
    return (TYPE)(native_divide(x, (float)(SCALE)));
#endif
}

#if RADIUS == 0
#define DFT_T(d, s)  dft_1((d), (s))
#define IDFT_T(d, s) idft_1((d), (s))
#elif RADIUS == 1
#define DFT_T(d, s)  dft_3((d), (s))
#define IDFT_T(d, s) idft_3((d), (s))
#elif RADIUS == 2
#define DFT_T(d, s)  dft_5((d), (s))
#define IDFT_T(d, s) idft_5((d), (s))
#else
#define DFT_T(d, s)  dft_7((d), (s))
#define IDFT_T(d, s) idft_7((d), (s))
#endif

#define TRANSPOSE_STRIDE ((WARP_SIZE % BLOCK_SIZE == 0) ? (BLOCK_SIZE + 1) : BLOCK_SIZE)
#define TILES_PER_GROUP ((WARPS_PER_BLOCK * WARP_SIZE) / BLOCK_SIZE)

__attribute__((reqd_work_group_size(WARPS_PER_BLOCK * WARP_SIZE, 1, 1)))
kernel void fused(
    global float * restrict dstp0,
    global const TYPE * restrict srcp0,
    const int spatial_off,
    const int padded_off,
    const int width,
    const int height,
    global const float * restrict window,
    global const float * restrict window_freq,
    global const float * restrict sigma_array
) {
    global float * restrict dstp = dstp0 + spatial_off;
    global const TYPE * restrict srcp = srcp0 + padded_off;

    const int radius = RADIUS;
    const int block_size = BLOCK_SIZE;
    const int block_step = BLOCK_STEP;

    const int horizontal_num = calc_pad_num(width, block_size, block_step);
    const int vertical_num = calc_pad_num(height, block_size, block_step);
    const int horizontal_size = calc_pad_size(width, block_size, block_step);
    const int vertical_size = calc_pad_size(height, block_size, block_step);
    const int num_blocks = vertical_num * horizontal_num;

    local float2 shared_transpose_buffer[TILES_PER_GROUP * BLOCK_SIZE * TRANSPOSE_STRIDE];
#if ZERO_MEAN
    local float gf_all[TILES_PER_GROUP];
#endif

    const int tid = (int)get_local_id(0);
    const int tile = tid / block_size;
    const int lane_id = tid % block_size;
    local float2 *transpose_buffer = &shared_transpose_buffer[tile * block_size * TRANSPOSE_STRIDE];
    local float *tbf = (local float *)transpose_buffer;

    const int block_id = (int)get_group_id(0) * TILES_PER_GROUP + tile;
    const int active = block_id < num_blocks;
    const int bid = active ? block_id : 0;
    const int ix = bid % horizontal_num;
    const int iy = bid / horizontal_num;

    float2 thread_data[(2 * RADIUS + 1) * BLOCK_SIZE];

    #pragma unroll
    for (int i = 0; i < 2 * radius + 1; i++) {
        global const TYPE *src = &srcp[(i * vertical_size + iy * block_step) * horizontal_size + ix * block_step];
        float *local_thread_data_f = (float *)&thread_data[i * block_size];
        #pragma unroll
        for (int j = 0; j < block_size; j++) {
            local_thread_data_f[j] = to_float(src[j * horizontal_size + lane_id]) * window[(i * block_size + j) * block_size + lane_id];
        }
    }

    #pragma unroll
    for (int i = 0; i < 2 * radius + 1; i++) {
        float2 *local_thread_data = &thread_data[i * block_size];
        float *local_thread_data_f = (float *)local_thread_data;

        barrier(CLK_LOCAL_MEM_FENCE);
        #pragma unroll
        for (int j = 0; j < block_size; j++) {
            tbf[j * TRANSPOSE_STRIDE + lane_id] = local_thread_data_f[j];
        }

        barrier(CLK_LOCAL_MEM_FENCE);
        #pragma unroll
        for (int j = 0; j < block_size; j++) {
            local_thread_data_f[j] = tbf[lane_id * TRANSPOSE_STRIDE + j];
        }

        barrier(CLK_LOCAL_MEM_FENCE);
        rdft_16(local_thread_data_f);
    ...
    if (lane_id < block_size / 2 + 1) {
        ...
        for (int i = 0; i < block_size; i++) {
            DFT_T((float *)&thread_data[i], block_size);
        }
    }

#if ZERO_MEAN
    if (lane_id == 0) {
        gf_all[tile] = native_divide(thread_data[0].x, window_freq[0]);
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    const float gf = gf_all[tile];
#endif
    float2 *local_thread_data = &thread_data[radius * block_size];
    float *local_thread_data_f = (float *)local_thread_data;
    if (lane_id < block_size / 2 + 1) {
        #pragma unroll
        for (int i = 0; i < 2 * radius + 1; i++) {
            ...
            for (int j = 0; j < block_size; j++) {
                ...
                filter(&local_data, lane_id, j, i, sigma_array);
        ...
        #pragma unroll
        for (int i = 0; i < block_size; i++) {
            IDFT_T((float *)&thread_data[i], block_size);
        }

        idft_16(local_thread_data_f, 1);
    ...
    barrier(CLK_LOCAL_MEM_FENCE);
    irdft_16(local_thread_data_f);

    #pragma unroll
    for (int j = 0; j < block_size; j++) {
        tbf[j * TRANSPOSE_STRIDE + lane_id] = local_thread_data_f[j == 0 ? j : block_size - j];
    }

    barrier(CLK_LOCAL_MEM_FENCE);
    if (active) {
        global float *local_dst = &dstp[block_id * block_size * block_size];
        #pragma unroll
        for (int j = 0; j < block_size; j++) {
            local_dst[j * block_size + lane_id] = tbf[lane_id * TRANSPOSE_STRIDE + j];
        }
    }
}

__attribute__((reqd_work_group_size(WARP_SIZE, WARPS_PER_BLOCK, 1)))
kernel void col2im(
    global TYPE * restrict dst0,
    ...
    global TYPE * restrict dst = dst0 + dst_off;
    global const float * restrict src = src0 + src_off;

    int radius = RADIUS;
    int block_size = BLOCK_SIZE;
    int block_step = BLOCK_STEP;
    ...
    for (int i = i1; i <= i2; i++) {
        ...
        for (int j = j1; j <= j2; j++) {
            ...
            int src_offset = ((i * horizontal_num + j) * block_size + offset_y) * block_size + offset_x;
            int window_offset = (radius * block_size + offset_y) * block_size + offset_x;
...
kernel void reflect_pad(
    global TYPE * restrict dst0,
    const int dst_off,
    global const TYPE * restrict src0,
    const int src_off,
    const int width,
    const int height,
    ...
    global TYPE * restrict dst = dst0 + dst_off;
    global const TYPE * restrict src = src0 + src_off;
